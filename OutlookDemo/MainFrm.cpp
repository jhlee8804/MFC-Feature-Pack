...
BOOL CMainFrame::CreateOutlookBar(CMFCOutlookBar& bar, UINT uiID, CMFCOutlookBarPane& pane1, CMFCOutlookBarPane& pane2, CMFCShellTreeCtrl& tree, CCalendarBar& calendar, int nInitialWidth) {
  CWindowDC dc(NULL);
  const BOOL bIsHiColor = dc.GetDeviceCaps(BITSPIXEL) > 16;
...
int CMainFrame::FindFocusedOutlookWnd(CMFCOutlookBarTabCtrl** ppOutlookWnd) {
  ...
  *ppOutlookWnd = NULL;
  ...
  CWnd* pWnd = m_dockManager.PaneFromPoint(pt, 0);
